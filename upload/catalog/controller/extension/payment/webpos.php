...
class ControllerExtensionPaymentWebpos extends Controller {
		...
		$order_data = array();
			...
			foreach ($results as $result) {
				if ($this->config->get('total_'. $result['code'] . '_status')) {
					...
					$this->{'model_extension_total_' . $result['code']}->getTotal(['totals' => &$order_data['totals'], 'total' => &$total, 'taxes'=> &$taxes]);
				}
			}
	...
	public function instalments() {
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/extension/payment/webpos_instalment')) {
			$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/extension/payment/webpos_instalment', $data));
		} else {
	...
	public function callback(){
		...
		if ($method_response['result']==1){
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/extension/payment/webpos_success')) {
				$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/extension/payment/webpos_success', $data));
			} else {
			...
			$data['message']=$method_response['message'];
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/extension/payment/webpos_failure')) {
				$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/extension/payment/webpos_failure', $data));
			} else {
