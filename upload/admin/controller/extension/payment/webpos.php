...
class ControllerExtensionPaymentWebPos extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('payment_webpos', $this->request->post);
		...
		$data['tab_add_url'] = $this->url->link('extension/module/webposbuilder', 'user_token=' . $this->session->data['user_token'], true);
		...
		'href' => $this->url->link('extension/payment/webpos', 'user_token=' . $this->session->data['user_token'], true)
		);

		$data['action'] = HTTPS_SERVER . 'index.php?route=extension/payment/webpos&user_token=' . $this->session->data['user_token'];
		...
		$banks=$this->model_extension_module_webposbuilder->getbanks(array('sort'=>'bank_id'));
		//fix key sort order
		$new_banks=array();
		...
		if (isset($this->request->post['payment_webpos_banks_info'])) {
			$data['banks_info'] = $this->request->post['payment_webpos_banks_info'];
		} else {
			$data['banks_info'] = $this->config->get('payment_webpos_banks_info');
		}
		...
		//var_dump($banks);
		//
		if (isset($this->request->post['payment_webpos_mode'])) {
			$data['payment_webpos_mode'] = $this->request->post['payment_webpos_mode'];
		} else {
			$data['payment_webpos_mode'] = $this->config->get('payment_webpos_mode');
		}

		if (isset($this->request->post['payment_webpos_other_id'])) {
			$data['payment_webpos_other_id'] = $this->request->post['payment_webpos_other_id'];
		} else {
			$data['payment_webpos_other_id'] = $this->config->get('payment_webpos_other_id');
		}

		if (isset($this->request->post['payment_webpos_order_status_id'])) {
			$data['payment_webpos_order_status_id'] = $this->request->post['payment_webpos_order_status_id'];
		} else {
			$data['payment_webpos_order_status_id'] = $this->config->get('payment_webpos_order_status_id');
		}
		...
		if (isset($this->request->post['payment_webpos_status'])) {
			$data['payment_webpos_status'] = $this->request->post['payment_webpos_status'];
		} else {
			$data['payment_webpos_status'] = $this->config->get('payment_webpos_status');
		}

		if (isset($this->request->post['payment_webpos_total'])) {
			$data['payment_webpos_total'] = $this->request->post['payment_webpos_total'];
		} else {
			$data['payment_webpos_total'] = $this->config->get('payment_webpos_total');
		}

		if (isset($this->request->post['payment_webpos_sort_order'])) {
			$data['payment_webpos_sort_order'] = $this->request->post['payment_webpos_sort_order'];
		} else {
			$data['payment_webpos_sort_order'] = $this->config->get('payment_webpos_sort_order');
		}
