...
class get7243dHosting {
    ...
    private function createHash($clientId,$storekey,$okUrl,$failUrl,$oid,$amount,$rnd,$transactionType,$instalment,$callbackUrl) {
        ...
        // callbackUrl is not necessary unless your bank says it is.
        // $hashstr = $clientId . $oid . $amount . $okUrl . $failUrl . $transactionType . $instalment . $rnd . $callbackUrl . $storekey;
    ...
    private function createForm($bank) {
        $rnd = microtime();
        $clientId=$bank['get724_client_id'];
        $storekey=$bank['get724_3D_storekey'];
        $okUrl=$bank['success_url'];
        $failUrl=$bank['fail_url'];
        $oid=$bank['order_id'];
        $amount=$bank['total'];
        $callbackUrl=$bank['fail_url'];
        ...
        $inputs=array('clientid'=>$bank['get724_client_id'],
                      'amount'=>$amount,
                      'oid'=>$oid,
                      'okUrl'=>$okUrl,
                      'failUrl'=>$failUrl,
            //'callbackUrl'=>$callbackUrl,
                      ...
                      'lang'=>"tr",
                      'currency'=>"949",
                      'firmaadi'=>$bank['get724_3D_storename'],
                      'refreshtime'=>"10",
        ...
        $action='';
        if ($bank['mode']=='live') {
            $action=$bank['get724_3D_url'];
        } else if ($bank['mode']=='test') {
            $action=$bank['get724_test_url'];
        }
